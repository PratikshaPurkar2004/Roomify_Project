...
function Dashboard() {
  ...
  const graphData = React.useMemo(() => {
    const days = [];
    const now = new Date();
    // Realistic distribution wave for 7 days
    const dist = [0.10, 0.15, 0.05, 0.20, 0.25, 0.15, 0.10]; 
    
    let sumV = 0, sumM = 0, sumR = 0;
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const r = stats.requests || 0;

      const dayV = Math.floor(v * dist[6 - i]);
      const dayM = Math.floor(m * dist[6 - i]);
      const dayR = Math.floor(r * dist[6 - i]);
    ...
    // Fix remainder to guarantee sum exactly matches the DB totals
    if (days.length > 0) {
      days[6].views += Math.max(0, (stats.views || 0) - sumV);
      days[6].matches += Math.max(0, (stats.matches || 0) - sumM);
      days[6].requests += Math.max(0, (stats.requests || 0) - sumR);
    }
    return days;
  }, [stats]);
