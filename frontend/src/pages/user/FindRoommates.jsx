...
//                       navigate("/dashboard/chat", { state: { selectedUserId: user.id } });
//                     }}
...
export default function FindRoommates() {
  ...
  const [rooms, setRooms] = useState([]);
  const [results, setResults] = useState([]);
  const [searchType, setSearchType] = useState("roommates");
  ...
  // Fetch user preferences + requests
  useEffect(() => {
    const userId = localStorage.getItem("userId");
    if (!userId) return;
    ...
    fetch(`http://localhost:5000/api/requests/sent/${userId}`)
      .then(res => res.json())
      .then(data => {
        if (data.success) setSentRequests(data.sentRequests);
      });

    fetch(`http://localhost:5000/api/requests/accepted-ids/${userId}`)
      .then(res => res.json())
      .then(data => {
        if (data.success) setAcceptedIds(data.acceptedIds);
      });
  }, []);
  ...
  useEffect(() => {
    let listToFilter = searchType === "roommates" ? roommates : rooms;

    const filtered = listToFilter.filter(item => {
      const itemLocation = String(item.location || "").toLowerCase();
      const matchesCity = city === "" || itemLocation.includes(city.toLowerCase());

      if (searchType === "roommates") {
        const userGender = String(item.gender || "").toLowerCase();
        const userRent = item.rent != null ? Number(item.rent) : null;

        const matchesGender = gender === "" || userGender === gender.toLowerCase();
        const matchesBudget = budget === "" || (userRent !== null && userRent <= Number(budget));

        return matchesCity && matchesGender && matchesBudget;
      } else {
        const roomRent = Number(item.rent);
        const matchesBudget = budget === "" || roomRent <= Number(budget);
  ...
  const handleRequest = async (receiverId, name) => {
    ...
    try {
      ...
      if (data.success) {
        setToastMessage(`Request sent to ${name} ✅`);
        setSentRequests(prev => [...prev, receiverId]);
      } else {
  ...
  <div className="roommate-page">
    ...
    <div className="roommate-container">
      ...
      <div className="rm-search-box">
        <div className="rm-input-group">
          <MapPin className="rm-input-icon" size={18} />
          <select value={city} onChange={(e) => setCity(e.target.value)}>
            <option value="">All Cities</option>
            {[...new Set(roommates.map(u => u.location).filter(Boolean))].map(c => (
        ...
        <div className="rm-input-group">
          <Wallet className="rm-input-icon" size={18} />
          <input
            type="number"
            placeholder="Budget"
            value={budget}
      ...
      <div className="roommate-grid">
        {results.map((item) => {
          if (searchType === "roommates") {
            const match = getMatch(item.preferences);
            const hasSent = sentRequests.includes(item.id);
            ...
            return (
              <div className="roommate-card-modern" key={item.id}>
                ...
                <div className="rm-card-body">
                  <div className="rm-detail">
                    <Wallet className="rm-icon" size={16} />
                    <span>₹ {item.rent}</span>
                  </div>

                  <div className="rm-detail">
                    <User className="rm-icon" size={16} />
                ...
                <div className="rm-card-actions">
                  ...
                  <button
                    className={`rm-btn ${
                      hasSent ? "rm-btn-sent" : "rm-btn-request"
                    }`}
                    disabled={hasSent}
